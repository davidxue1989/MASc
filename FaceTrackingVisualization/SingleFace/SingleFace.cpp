    ...
    if (pApp)
    {
        ...
        if (pResult && SUCCEEDED(pResult->GetStatus()))
        {
			...
			*/
			...
			ofstream myfile;
			myfile.open("pose.csv", ios::out | ios::app);
			myfile << tranx << "," << trany << "," << tranz << "," << dirx << "," << diry << "," << dirz << endl;
			myfile.close();


			//determine if gazing at an object
			Vector M (VZERO);
			Matrix cov_inv(VZERO, VZERO, VZERO);
			FLOAT zscore;
			Vector X(VZERO);
			Vector P0(tranx, trany, tranz);
			Vector U(dirx, diry, dirz);
			
			M.set(-0.0558, -0.0252, 0.6186);
			cov_inv.set(Vector(1984, 72, 1513), Vector(72, 1677, 3156), Vector(1513, 3156, 44455));
			
			X = P0 + U*dot((M - P0), U);
			zscore = dot(X-M, cov_inv*(X-M));
			BOOL gazing = zscore < 50 ? true : false;


			IFTImage* kinectImage = pApp->m_FTHelper.m_colorImage;
			IplImage* cvImg = cvCreateImageHeader(cvSize(kinectImage->GetWidth(),
				kinectImage->GetHeight()), IPL_DEPTH_8U, 4);
			cvImg->imageData = (char*) pApp->m_FTHelper.m_colorImage->GetBuffer();
			stringstream ss, ss1,ss2, ss3, ss4;
			//dxdebug: displaying the rotation angles on image
			ss << "pitch: " << rotationXYZ[0] << " yaw: " << rotationXYZ[1] << " roll: " << rotationXYZ[2];
			//ss1 << " P0:" << P0.x << "," << P0.y << "," << P0.z << ")";
			//ss2 << " U:" << U.x << "," << U.y << "," << U.z << ")";
			//ss3 << " X:(" << X.x << "," << X.y << "," << X.z << ")";
			ss4 << " Z-score:" << zscore << (gazing ? "GAZING" : "");
			CvFont font;
			cvInitFont(&font, CV_FONT_HERSHEY_SIMPLEX, 0.5, 0.5);
			cvPutText(cvImg, ss.str().c_str(), cvPoint(100, 50), &font, cvScalar(0,255,0));
			//cvPutText(cvImg, ss1.str().c_str(), cvPoint(100, 250), &font, cvScalar(0, 255, 0));
			//cvPutText(cvImg, ss2.str().c_str(), cvPoint(100, 350), &font, cvScalar(0, 255, 0));
			//cvPutText(cvImg, ss3.str().c_str(), cvPoint(100, 450), &font, cvScalar(0, 255, 0));
			cvPutText(cvImg, ss4.str().c_str(), cvPoint(100, 150), &font, cvScalar(0, 255, 0));
			memcpy(kinectImage->GetBuffer(), cvImg->imageData, kinectImage->GetBufferSize());
