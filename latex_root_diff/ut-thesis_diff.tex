...
%DIF DEL C:\Users\David\Documents\GitHub\MASc\latex_root_before_committee_rev\ut-thesis.tex   Sun Aug 16 23:25:34 2015
%DIF ADD C:\Users\David\Documents\GitHub\MASc\latex_root\ut-thesis.tex                        Wed Sep  2 12:26:07 2015
%%
...
\subsection{Discussion}
This lack of impact on task performance in Wainer et al.'s study may be due to the learning effect when solving the puzzle and the novelty factor of using the robot.  Also, the use of such SAR on the clinical population and on clinically relevant tasks may yield a better result because of greater need for user engagement and user compliance in that context.  For our thesis, we will investigate the impact of \DIFdelbegin \DIFdel{an embodied }\DIFdelend \DIFaddbegin \DIFadd{a prompting agent embodied as a humanoid }\DIFaddend robot on the clinical population of children with ASD in the task of hand-washing, where user's lack of compliance to prompts and \DIFdelbegin \DIFdel{of }\DIFdelend engagement in task executions are observed problems.  One thing to note is, we cannot conduct self-reflective surveys due to the nature of the children with ASD population.  Therefore, we will rely on objective measures for user engagement such as verbal responses and physical behaviors (proximity, posture, eye gaze\DIFdelbegin \DIFdel{and ﬁxation) \mbox{%DIFAUXCMD
\cite{mataric2005role} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{) \mbox{%DIFAUXCMD
\cite{mataric2005role} }%DIFAUXCMD
as well as subjective observations from the parents and the researcher}\DIFaddend .
...
Participants were to be children between the ages of 4 to 15 with a diagnosis of ASD, and their parent. \DIFdelbegin \DIFdel{Six }\DIFdelend \DIFaddbegin \DIFadd{Three }\DIFaddend children would be recruited. This sample size is typical for studies of this nature \DIFaddbegin \DIFadd{(i.e. HRI case study }\DIFaddend for children with ASD\DIFdelbegin \DIFdel{. For example, a pilot study by Bimbrahw }\DIFdelend \DIFaddbegin \DIFadd{), e.g. Kozima }\DIFaddend et al. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{bimbrahw2012investigating} }%DIFAUXCMD
...
The measure \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend Total Number of Times Child Smiles\DIFdelbegin \DIFdel{" }\DIFdelend \DIFaddbegin \DIFadd{'' }\DIFaddend is shown in Plot \ \ref{fig:12TotalNumberofTimesChildSmiles}.  In it, parent alone phase (A) levels at 1.5, robot alone phase (first Phase B) levels at 0.5, robot parent phase (C) has a large spread and averages around 3, and robot alone repeat phase (second Phase B) also has a large spread and averages around 4.  It shows that the child smiles much more in later phases compared to earlier phases, and particularly, smiles in the repeat phase more than in robot alone phase.
\begin{figure} [h]
	\centering
...
The measure \DIFdelbegin \DIFdel{"}\DIFdelend \DIFaddbegin \DIFadd{``}\DIFaddend Total Number of Times Child Murmurs\DIFdelbegin \DIFdel{" }\DIFdelend \DIFaddbegin \DIFadd{'' }\DIFaddend is shown in Plot \ \ref{fig:13TotalNumberofTimesChildMurmurs}.  In it, parent alone phase (A) has a large spread, averaging around 4.  Robot alone phase (first Phase B) levels at 0.5.  Robot parent phase (C) has a large spread, averaging around 4.  Robot alone repeat phase (second Phase B) levels at 2.  It shows that the child murmurs much more often when the parent is present.  Also, child murmurs in the repeat phase more than the robot alone phase.
\begin{figure} [h]
	\centering
