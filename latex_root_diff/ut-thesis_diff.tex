...
One major objective of this thesis is to investigate the impacts that using a humanoid prompting agent has on the visual attention, prompt compliance, and task performance of children with ASD during hand-washing activities.  \DIFdelbegin %DIFDELCMD < 
...
\DIFaddbegin \section{\DIFadd{Case Study Format}}
...
.  This is because the case study format is exploratory as opposed to evaluative in nature, employs both quantitative and qualitative data analyses, and deals with a small number of subjects \mbox{%DIFAUXCMD
\cite{yin2013case}}%DIFAUXCMD
...
Each visit would take about an hour to an hour and a half. The child was \DIFdelbegin \DIFdel{to be }\DIFdelend asked to wash his/her hands eight times for every visit, for a total of forty-eight trials per child.  This way, the consecutive trials conducted within a day was hopefully not too much for the child to cause fatigue.  Also, having sixteen trials per phase gives a sufficient sample size for both quantitative and qualitative \DIFdelbegin \DIFdel{(visual) }\DIFdelend analyses.  The child and his/her parent could take short breaks after each hand-washing \DIFdelbegin \DIFdel{trials}\DIFdelend \DIFaddbegin \DIFadd{trial}\DIFaddend .  The break could last as long as the child needed until he/she was willing to continue the trial. If the parent felt the need, they could leave and come back to finish the rest of the day's trials another day. They would not be withdrawn from the study unless requested.

The participating subjects would be randomly assigned one of the two phase orders: A-B-C and A-C-B.  This \DIFdelbegin \DIFdel{would reduce the confounding effect of learning }\DIFdelend \DIFaddbegin \DIFadd{was to try to reduce any learning effects }\DIFaddend when we compare between phase B and C \DIFaddbegin \DIFadd{as well as to see if the presence of phase C prior to phase B would have an effect on phase B's performance}\DIFaddend .
...
\DIFadd{The specific definitions for each measure will be discussed further in the analysis section \ref{sec:VideoDataAnalysisAndResults}.  The difference between prompts and prompt sections is discussed in \ref{sec:AnnotationFramework}.
...
\DIFaddbegin \bibitem{dautenhahn1999robots}
\DIFadd{Kerstin Dautenhahn.
}\newblock \DIFadd{Robots as social actors: Aurora and the case of autism.
}\newblock \DIFadd{In }{\em \DIFadd{Proc. CT99, The Third International Cognitive Technology
  Conference, August, San Francisco}}\DIFadd{, volume 359, page 374, 1999.
}

\DIFaddend \bibitem{dautenhahn2004towards}
