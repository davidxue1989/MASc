...
*/
				...
				if (m_pDrawDataStreams->ValidateFaceBoxAndPoints(&faceBox, facePoints)) {
					//dx: save the location and pose of the head [x,y,z,u,v,w]
					//writing to file
					std::string pathname = "pose.csv";
					std::fstream file(pathname, std::ios::out | std::ios::app);
					file.exceptions(std::ios::badbit | std::ios::failbit);
					//change from quaterion angle to pitch yaw roll (is that euler angles?)
					int pitch, yaw, roll;
					ImageRenderer::ExtractFaceRotationInDegrees(&faceRotation, &pitch, &yaw, &roll);
					file << headPivot.X << "," << headPivot.Y << "," << headPivot.Z << "," << pitch << "," << roll << "," << yaw << "\n";
					file.close();
				}
