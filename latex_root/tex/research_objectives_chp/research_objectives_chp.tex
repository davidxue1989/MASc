...
Our overall goal is to increase a child with ASD's engagement level during COACH prompting for hand-washing tasks, and thus improving prompt compliance and task completion rate.  Our approach is:
\begin{enumerate}
	...
	\item to automatically track the VFOA of the child so that the robot can call out child's name with a waving gesture or blink its LEDs for getting child's attention if the child is not looking at the robot before prompting
