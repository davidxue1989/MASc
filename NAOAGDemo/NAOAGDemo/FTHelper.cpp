...
HRESULT FTHelper::Init(HWND hWnd, FTHelperCallBack callBack, PVOID callBackParam, FTHelperCallBackTrackLost callBackTrackLost, PVOID callBackTrackLostParam,
	NUI_IMAGE_TYPE depthType, NUI_IMAGE_RESOLUTION depthRes, BOOL bNearMode, BOOL bFallbackToDefault, NUI_IMAGE_TYPE colorType, NUI_IMAGE_RESOLUTION colorRes, BOOL bSeatedSkeletonMode)
{
	...
	m_CallBackTrackLost = callBackTrackLost;
	m_CallBackTrackLostParam = callBackTrackLostParam;
...
BOOL FTHelper::SubmitFraceTrackingResult(IFTResult* pResult)
{
	if (pResult != NULL && SUCCEEDED(pResult->GetStatus()))
	{
		...
		//dx sep 02, 2013: disabled visualization for MFC integration
		//    if (m_DrawMask)
		//    {
		//        FLOAT* pSU = NULL;
		//        UINT numSU;
		...
		//hr = VisualizeFacetracker(m_colorImage, pResult, 0x00FF6400);
		//            ftModel->Release();
		//        }
		//    }
	}
...
BOOL FTHelper::FaceTrackingLost() {
	if (m_CallBackTrackLost)
	{
		(*m_CallBackTrackLost)(m_CallBackTrackLostParam);
		return TRUE;
	}
	return FALSE;
}
