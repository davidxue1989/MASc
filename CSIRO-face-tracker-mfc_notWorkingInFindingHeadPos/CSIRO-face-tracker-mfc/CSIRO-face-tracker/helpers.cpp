...
std::string dirname(std::string pathname) {
	...
	char sep = '/';
#endif
	std::string rv = pathname.substr(0, pathname.find_last_of(sep));
	return rv;
}
...
std::string basename(std::string pathname) {
	...
	char sep = '/';
#endif
	std::string rv = pathname.substr(pathname.find_last_of(sep)+1);
	return rv;
}
