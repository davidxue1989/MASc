...
run_cam_mode(int cam_id)
{
	...
	while (true) {
		...
		if (result >= cfg.tracking_threshold) {
			...
			//getting the location by finding centroid of all points
			cv::Point3_<double> loc;
			int sz = 0;
			for (int i = 0; i < shape3D.size(); i++) {
				cv::Point3_<double> pt = shape3D[i];
				loc.x += pt.x;
				loc.y += pt.y;
				loc.z += pt.z;
				sz++;
			...
			file << loc.x << "," << loc.y << "," << loc.z << "," << pose.pitch << "," << pose.roll << "," << pose.yaw << "\n";
			file.close();
		}
		else {
			tracker->Reset();
